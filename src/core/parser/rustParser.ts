  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (useMatch) {
      ...
      imports.push({
        ...
        isStandardLib: source.startsWith('std::') || source.startsWith('core::'),
        line: lineNum
      });
      continue;
    }
    ...
    // Impl block
    const implMatch = trimmed.match(/impl(?:<.*?>)?\s+(?:.*?\s+for\s+)?([A-Za-z0-9_]+)/);
    if (implMatch) {
      currentImpl = implMatch[1];
    }

    // Function / Method
    const fnMatch = line.match(/(?:pub\s+)?(?:async\s+)?(?:unsafe\s+)?fn\s+([A-Za-z0-9_]+)\s*(?:<.*?>)?\s*\((.*?)\)(?:\s*->\s*([^{]+))?/);
    if (fnMatch) {
      ...
      const isMethod = currentImpl !== null && paramsRaw.includes('self');
      ...
      if (/File::|std::fs|read_to_string|write_all|TcpStream|reqwest|tokio::fs/.test(bodyCode)) {
        tags.push(/reqwest|TcpStream|http/.test(bodyCode) ? 'net' : 'io');
      }
