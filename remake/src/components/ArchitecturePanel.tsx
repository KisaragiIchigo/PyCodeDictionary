      ...
      {blueprints.length > 0 && (
        <div className="space-y-2.5">
          ...
          {blueprints.map((bp, idx) => {
            const combinedCode = `${bp.extractedStepCodes.map(s => s.code).join('\n\n')}\n\n${bp.orchestratorCode}`;
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    {onApplyRefactor && (
                      <button
                        onClick={() => onApplyRefactor(combinedCode)}
                        className="flex items-center gap-1 px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-[10px] font-mono hover:bg-emerald-500/30 transition-all"
                        title="生成された設計コードをエディタに直接適用"
                      >
